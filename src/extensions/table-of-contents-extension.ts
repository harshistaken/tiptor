            ...
            appendTransaction: (transactions, _oldState, newState) => {
                ...
                if (transactions.some((transaction) => transaction.docChanged)) {
                    newState.doc.descendants((node, pos) => {
                        if (this.options.anchorTypes?.includes(node.type.name) && node.textContent.trim().length > 0) {
    ...
    onCreate() {
        ...
        if (this.options.scrollParent) {
            this.storage.scrollHandler = () => {
                ...
                const scrollPosition = scrollParent instanceof Window ? scrollParent.scrollY : scrollParent.scrollTop;
...
export const getHeadlineLevel: GetTableOfContentLevelFn = (headline, previousItems) => {
    ...
    const lastSameLevel = [...previousItems].reverse().find((item) => item.originalLevel <= headline.node.attrs.level);
...
export const getHierarchicalIndexes: GetTableOfContentIndexFn = (headline, previousItems, currentLevel) => {
